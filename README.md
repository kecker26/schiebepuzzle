...
- Autosave, kreative KI-Titel fuer neue Spielstaende, motivbasierte Titel-Wiederverwendung, maximal 30 aktive Spielstaende mit 5er-Seitennavigation, Resume-Flow, Recovery-Dialog und Last-Session-Wiederaufnahme. Der Startscreen zeigt zusaetzlich motivweit deduplizierte Challenge-Medaillen, oeffnet die Galerie direkt mit Medaillenfiltern, weist beim Fortsetzen auf das naechste erreichbare Challenge-Ziel hin und bietet eine Schnellaktion zur gefilterten, nach Upgrade-Potenzial sortierten Medaillen-Jagd.
...
- Soft-Ziele sind feste synthetische Rasterziele fuer ein neues Motiv/Raster/Crop/Startboard und werden erst beim Abschluss als `estimatedChallengeTarget` gespeichert. Startbrett-Heuristik veraendert diese Zielwerte nicht; nach ausreichend cleanen Laeufen passt nur der persoenliche Median die Schaetzung an. Fruehere passende cleane Laeufe, die Zeit und Zuege dieser Schaetzung strikt unterbieten, gelten als vorhandene synthetische Serie; ein erster cleaner Soft- oder Qualifikationslauf erzeugt nur dann die echte Vorlage, wenn Zeit und Zuege strikt unterboten werden. Medaillenlaeufe laufen nur gegen echte Vorlagen; `challengeMedal` wird ausschliesslich fuer absolut cleane Medaillenlaeufe gesetzt. Bestehende Medaillen werden nicht neu bewertet, auch wenn neue Regeln gelten.
- Qualifikations- und Medaillenlaeufe werden nicht als fortsetzbare Spielstaende gespeichert; Soft-Ziele autosaven wie normale Laeufe. Aeltere geladene Spielstaende koennen ihr aktives Ziel noch rueckwaertskompatibel als `challengeTarget` enthalten und werden beim Abbruch geloescht, wenn sie ein gesperrter Zielmodus sind. Galerieeintraege speichern ihre `challengeTargetId` auch bei medaillenlosen Soft-/Qualifikations-/Challenge-Abschluessen. Neue Medaillen-Vorlagen muessen ein cleanes Detailprofil haben; assistierte Startzustaende bleiben als Uebung replaybar. Startzustandslaeufe mit demselben Startbrett werden zuerst der passenden synthetischen oder echten Zielserie zugeordnet, bleiben ausserhalb der Medaillenwertung und teilen dort einen gemeinsamen Uebungsstart; Challenge-Serien ohne echte Vorlage oder ohne solche Zusatzlaeufe bleiben trotzdem ueber ihren gespeicherten Startzustand als Uebung replaybar. Neutrale Startzustand-Serien sind nur noch ein Legacy-Fallback fuer alte oder inkonsistente Laeufe ohne Zielserienbezug.
