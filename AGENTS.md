...
- Neue Medaillenlaeufe mit aktivem `challengeTarget` werden nicht als fortsetzbare Spielstaende autosaved. Beim Abbruch muss ein eventuell bereits vorhandener aelterer Challenge-Spielstand geloescht werden; Galerie-Laeufe speichern weiterhin optional `challengeTargetId` und `challengeMedal`. Fehlende Ziellaeufe duerfen historische Medaillen nicht invalidieren.
- Aktive Medaillenlaeufe sperren im Puzzle-Screen alle Spielhilfen inklusive Vorschau-Umschaltung und Zugverlauf; Zielbild und strategischer Fokus rechts bleiben verfuegbar. Buttons, Tastaturkuerzel, Kontextmenue und Handler muessen dieselbe Sperre beachten.
- Challenge-Medaillen sind exklusiv gestaffelt: Bronze verbessert genau ein Ziel, Silber beide Ziele strikt, Gold beide Ziele um mindestens 20 Prozent und Diamant ergaenzt Gold um ein exaktes Solver-Optimum. Bestehende gespeicherte Medaillen werden nicht rueckwirkend neu bewertet.
- Challenge-Serienkarten im Galerie-Detail sind ueber `AnimatedCollapse` einzeln einklappbar; ihre zugehoerigen Versuche stehen nach Bestwert sortiert, waehrend die Medaillen-Entwicklung chronologisch bleibt.
...
- Laeufe mit gleichem gespeicherten Startbrett, aber ohne Challenge-Bezug, werden im Galerie-Detail als neutrale Startzustand-Serien gruppiert statt als eigenstaendige Laeufe. Bereits eine einzelne cleane Vorlage bildet eine solche Serie und zeigt ihren getrennten Uebungslauf-Bereich als leere Liste. Jede Serie besitzt eine eigene einklappbare Karte mit genau einer Startbrett-Vorschau und kompakten Laufzeilen; ohne cleane Vorlage startet die gemeinsame Aktion im Replay-Modus `practice`, sobald ein Lauf clean geloest wurde, wird dieser priorisiert und die gemeinsame Aktion muss ihn als Medaillen-Vorlage herausfordern. Danach folgen die weiteren Laeufe nach Bestwert. Teilt ein solcher Lauf das Startbrett einer Challenge-Vorlage oder wurde die Vorlage mit Hilfe herausgefordert, wird er als verwandter Startzustandslauf an dieser Challenge-Serie angezeigt, bleibt ausserhalb der Medaillenwertung und nutzt mit den anderen verwandten Laeufen ebenfalls einen gemeinsamen Uebungsstart. Gemeinsame Uebungsstarts duerfen kein `challengeTarget` setzen.
