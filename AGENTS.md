...
- `src/services/RandomImageService.ts`, `PromptImageService.ts` und Provider-Dateien wie `NasaImageProvider.ts`, `MetMuseumImageProvider.ts`, `PicsumImageProvider.ts`, `OpenverseImageProvider.ts`, `LoremFlickrImageProvider.ts`, `FlickrImageProvider.ts`, `UnsplashImageProvider.ts`, `PixabayImageProvider.ts`, `PexelsImageProvider.ts`, `WikimediaImageProvider.ts`, `SmithsonianImageProvider.ts`, `ArtInstituteImageProvider.ts`, `ClevelandMuseumImageProvider.ts`, `GeneratedImageProvider.ts`: Zufallsbild-, Tag-Suchbild- und Prompt-Bild-Quellen. Die Zufallsbildauswahl mischt per Provider-Coordinator pro Anfrage gewichtet zufaellig und nutzt kurzlebige Cooldowns fuer fehlgeschlagene Anbieter.
...
- Bestehende API-Pfade (`/api/saves`, `/api/saves/:saveId/title`, `/api/stats`, `/api/gallery`, `/api/gallery/:entryId/analyze`, `/api/gallery/win-effect-tags`, `/api/collections`, `/api/image-proxy`, `/api/backup`, `/api/clipboard`, `/api/generated-image`, `/api/music`) nur aendern, wenn Frontend und lokale API gemeinsam angepasst werden.
...
- Bei Zufallsbild-Features Provider-Ausfaelle und CORS-/Lizenz-/Attributionsdaten defensiv behandeln. Externe Bildquellen ohne Browser-CORS duerfen nur ueber den eng erlaubten lokalen `/api/image-proxy`-Pfad eingebunden werden. Optionale Zufallsbild-Provider mit Frontend-Keys muessen in `.env.example` dokumentiert werden.
...
- Soft-Ziele entstehen nur fuer Motiv + Raster + Crop/Full + Startboard ohne bestehende echte oder geschaetzte Serie; passende fruehere cleane Laeufe, die Zeit und Zuege der Schaetzung strikt unterbieten, zaehlen dabei als vorhandene synthetische Serie. Sie verwenden feste synthetische Rasterziele ohne Startbrett-Heuristikanpassung, koennen sich nach ausreichend cleanen Laeufen ueber den persoenlichen Median anpassen, werden erst beim Abschluss als `estimatedChallengeTarget` gespeichert, autosaven normal und duerfen Hilfen nutzen. Eine echte Vorlage entsteht aus Soft- oder Qualifikationslaeufen nur, wenn Zeit und Zuege strikt unterboten werden.
